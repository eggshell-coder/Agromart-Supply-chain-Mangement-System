    ...
    try {
      if (authMode === 'signup') {
        ...
        if (data.session) {
          ...
          setSuccessMsg('Account created. Opening dashboard…'); setTimeout(() => onSuccess?.(), 500)
        } else {
          setSuccessMsg('Account created. Check your email to confirm your account, then sign in.'); setAuthMode('signin'); setPassword('')
        }
        return
      }
    ...
    <div style={{position:'relative',minHeight:'100dvh',width:'100%',overflow:'hidden',fontFamily:'Inter,-apple-system,sans-serif'}}>
      ...
      <div style={{position:'relative',zIndex:2,minHeight:'100dvh',display:'grid',gridTemplateColumns:'minmax(0,1fr) min(480px,94vw)',alignItems:'center',gap:32,padding:'clamp(24px,4vw,56px) clamp(16px,4vw,56px)'}}>
        ...
        <motion.div initial={{opacity:0,y:28}} animate={{opacity:1,y:0}} transition={{duration:.7,delay:.15}}><div style={{background:'rgba(240,255,246,.96)',backdropFilter:'blur(20px)',borderRadius:24,border:'1px solid rgba(255,255,255,.72)',boxShadow:'0 32px 80px rgba(4,18,10,.45)',padding:'32px 36px 36px'}}>
          <div style={{textAlign:'center',marginBottom:24}}><div style={{width:72,height:72,borderRadius:'50%',overflow:'hidden',margin:'0 auto 14px',background:'#efe4c5',border:'3px solid rgba(34,197,94,.3)'}}><img src={logoSrc} alt="AgroMart" onError={()=>setLogoSrc('/logo.png')} style={{width:'100%',height:'100%',objectFit:'cover',transform:'scale(1.4)'}}/></div><p style={{margin:0,fontSize:'.72rem',fontWeight:700,letterSpacing:'.15em',textTransform:'uppercase',color:'#2a7a4b'}}>🌾 AgroMart</p><h2 style={{margin:'8px 0 4px',fontSize:'1.55rem',fontWeight:800,color:'#0f2d1a'}}>{authMode==='signup'?'Create Staff Account':'Welcome Back'}</h2><p style={{margin:0,fontSize:'.85rem',color:'#4a7a5c'}}>{authMode==='signup'?'Create an account to operate AgroMart':'Sign in to continue your workflow'}</p></div>
