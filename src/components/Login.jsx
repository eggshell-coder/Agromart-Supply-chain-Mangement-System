...
export default function Login({ onSuccess }) {
    ...
    try {
      if (authMode === 'signup') {
        ...
        if (data.session) {
          ...
          setSuccessMsg('Account created. Opening dashboard…')
  ...
  return (
    <div style={{ position:'relative', minHeight:'100dvh', width:'100%', overflow:'hidden', fontFamily:'Inter,-apple-system,sans-serif' }}>
      ...
      <div style={{position:'relative',zIndex:2,minHeight:'100dvh',display:'grid',gridTemplateColumns:'minmax(0,1fr) min(480px,94vw)',alignItems:'center',gap:'32px',padding:'clamp(24px,4vw,56px) clamp(16px,4vw,56px)'}}>
        <div style={{color:'rgba(237,255,245,.97)',textShadow:'0 8px 24px rgba(6,42,22,.45)'}}>
          <p style={{margin:0,fontSize:'.85rem',fontWeight:700,letterSpacing:'.12em',textTransform:'uppercase',color:'rgba(163,230,153,.9)'}}>AgroMart Bangladesh</p>
          <AnimatePresence mode="wait"><motion.div key={slide} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-16}} transition={{duration:.6}}>
            <h1 style={{margin:'14px 0 0',fontSize:'clamp(2.1rem,4.2vw,4rem)',lineHeight:1.03,letterSpacing:'-.03em',fontWeight:800}}>Banglar matir golpo,<br/><span style={{color:'#a7f3d0'}}>{cur.label}.</span></h1>
            <p style={{margin:'18px 0 0',maxWidth:560,color:'rgba(220,255,237,.88)',fontSize:'clamp(.95rem,1.1vw,1.08rem)',lineHeight:1.7}}>{cur.desc}</p>
          </motion.div></AnimatePresence>
        </div>

        <motion.div initial={{opacity:0,y:28}} animate={{opacity:1,y:0}} transition={{duration:.7,delay:.15}}>
          <div style={{background:'rgba(240,255,246,.96)',backdropFilter:'blur(20px)',borderRadius:24,border:'1px solid rgba(255,255,255,.72)',boxShadow:'0 32px 80px rgba(4,18,10,.45)',padding:'32px 36px 36px'}}>
            <div style={{textAlign:'center',marginBottom:24}}>
              <div style={{width:72,height:72,borderRadius:'50%',overflow:'hidden',margin:'0 auto 14px',background:'#efe4c5',border:'3px solid rgba(34,197,94,.3)'}}><img src={logoSrc} alt="AgroMart" onError={()=>setLogoSrc('/logo.png')} style={{width:'100%',height:'100%',objectFit:'cover',transform:'scale(1.4)'}}/></div>
              <p style={{margin:0,fontSize:'.72rem',fontWeight:700,letterSpacing:'.15em',textTransform:'uppercase',color:'#2a7a4b'}}>🌾 AgroMart</p>
              <h2 style={{margin:'8px 0 4px',fontSize:'1.55rem',fontWeight:800,color:'#0f2d1a'}}>{authMode==='signup'?'Create Staff Account':'Welcome Back'}</h2>
              <p style={{margin:0,fontSize:'.85rem',color:'#4a7a5c'}}>{authMode==='signup'?'Create an account to operate AgroMart':'Sign in to continue your workflow'}</p>
            </div>

            {errorMsg && <div role="alert" style={{marginBottom:14,padding:'10px 12px',borderRadius:10,background:'#fee2e2',color:'#991b1b',fontSize:13}}>{errorMsg}</div>}
            {successMsg && <div role="status" style={{marginBottom:14,padding:'10px 12px',borderRadius:10,background:'#dcfce7',color:'#166534',fontSize:13}}>{successMsg}</div>}

            <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:14}}>
              {authMode==='signup' && <div><label style={{display:'block',fontSize:'.82rem',fontWeight:700,color:'#1e4d30',marginBottom:6}}>Full name</label><input value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Your full name" required style={{width:'100%',height:48,borderRadius:12,border:'1.5px solid rgba(34,197,94,.3)',padding:'0 16px',boxSizing:'border-box',color:'#0f2d1a'}}/></div>}
              ...
              <button type="submit" disabled={loading} style={{height:50,border:0,borderRadius:12,background:loading?'#86efac':'#16803d',color:'white',fontWeight:800,cursor:loading?'wait':'pointer'}}>{loading?'Please wait…':authMode==='signup'?'Create Account':'Sign In'}</button>
            </form>

            <div style={{textAlign:'center',marginTop:20,fontSize:13,color:'#4a7a5c'}}>{authMode==='signup'?'Already have an account?':'New to AgroMart?'} <button type="button" onClick={()=>{setAuthMode(m=>m==='signin'?'signup':'signin');setErrorMsg('');setSuccessMsg('')}} style={{border:0,background:'none',color:'#0f6b34',fontWeight:800,cursor:'pointer'}}>{authMode==='signup'?'Sign In':'Create Account'}</button></div>
            <p style={{margin:'14px 0 0',textAlign:'center',fontSize:11,color:'#6b8f76'}}>Farmers are business records entered by staff. They do not register or log in.</p>
          </div>
        </motion.div>
      </div>
    </div>
